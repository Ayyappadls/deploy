...
function createApp({ provider, providerLabel, rateLimit, store, nodeEnv }) {
 ...
 if(stage==='discover'){
   ...
   const acceptedRelationships=(data.relationships||[]).filter(r=>supportedEvidence(r,{evidence:combinedEvidence,evidenceOnFile:combinedEvidence},candidateSignals));
   const discovery=computeDiscoveryState({transcript:payload?.conversationTranscript||'',evidenceOnFile:combinedEvidence,signals:candidateSignals,openGaps:[...(payload?.openGaps||[]),...(data.knowledge_gaps||[])],contradictions:candidateContradictions,relationships:acceptedRelationships});
   data.relationships=acceptedRelationships;
