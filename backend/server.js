...
function createApp({ provider, providerLabel, rateLimit, store, nodeEnv }) {
  ...
  app.get('/api/mirror/:businessId', (req, res) => {
    ...
    if (!record) return res.status(404).json({ ok: false, error: { code: 'INVALID_REQUEST', message: 'No Business Mirror found for that id.' } });
  ...
  app.post('/api/mirror/:businessId', (req, res) => {
    ...
    if (typeof state !== 'object' || state === null) return res.status(400).json({ ok: false, error: { code: 'INVALID_REQUEST', message: 'state must be an object.' } });
    const saved = store.save(req.params.businessId, state);
    if (!saved) return res.status(404).json({ ok: false, error: { code: 'INVALID_REQUEST', message: 'No Business Mirror found for that id.' } });
    ...
    try {
      ...
      if (stage === 'discover') {
        const discovery = computeDiscoveryState({
          transcript: payload?.conversationTranscript || '',
          evidenceOnFile: payload?.evidenceOnFile || [],
        ...
        data.discoveryState = {
          ...
          activeSignalCount: discovery.activeSignalCount,
          materialUnknownCount: discovery.materialUnknownCount,
          contradictionCount: discovery.contradictionCount,
...
if (require.main === module) {
  ...
  const { provider, label: providerLabel } = selectProvider({ anthropicApiKey: ANTHROPIC_API_KEY, openaiApiKey: OPENAI_API_KEY, forceLocal, model: ANTHROPIC_MODEL, openaiModel: OPENAI_MODEL });
