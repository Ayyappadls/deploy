...
function buildCandidates({latest,evidence,signals,openGaps,contradictions,questions}){
 ...
 for(const e of(evidence||[]).filter(x=>['INFERRED','HYPOTHESIS'].includes(x.evidenceStatus)).slice(0,2))if(e.verificationQuestion&&!hasQuestion(questions,e.verificationQuestion))c.push({priority:95,objective:'verify_inference',layer:e.layer,text:e.verificationQuestion,why:'This is currently an inference or guess, so checking it will materially reduce uncertainty.'});
...
function computeDiscoveryState({transcript='',evidenceOnFile=[],signals=[],openGaps=[],contradictions=[]}){
 const latest=latestOwner(transcript),evidence=Array.isArray(evidenceOnFile)?evidenceOnFile:[],questions=dlsQuestions(transcript),meaningful=meaningfulEvidence(evidence),candidates=buildCandidates({latest,evidence,signals,openGaps,contradictions,questions});
 ...
 const complete=meaningful.length>=5&&signals.length>=1&&materialUnknowns.length===0&&!(contradictions||[]).length&&hasLayer(evidence,'owner')&&(hasLayer(evidence,'customer')||hasLayer(evidence,'offer'))&&hasLayer(evidence,'revenue');
 return{stage,latest,questions,evidenceCount:meaningful.length,activeSignalCount:signals.length,materialUnknownCount:materialUnknowns.length,contradictionCount:(contradictions||[]).length,candidates,nextBestQuestion:candidates[0]||null,complete};
}
