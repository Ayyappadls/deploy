...
function relationshipReadiness({evidence,signals,relationships=[]}){const explicit=(relationships||[]).filter(r=>supportedRelationship(r,evidence,signals));return{ready:explicit.length>0,explicitCount:explicit.length,crossLayerSignalCount:0};}
function decisionContextReady({latest,evidence}){const text=[latest,...(evidence||[]).filter(e=>e.layer==='owner').map(e=>e.normalizedMeaning||e.originalStatement||'')].join(' ');return/trying to|want to|need to|worried|concerned|decision|improve|change|fix|problem|what should|what do i do|where should/i.test(text);}
...
function materialPatternCandidates(latest,evidence,signals,questions){
 ...
 const econ=/revenue/.test(text)&&/(profit|margin|cash)/.test(text),service=/customization|customer-specific|custom work/.test(text)&&/support|implementation|engineering|delivery/.test(text),capacity=/capacity|team time|effort|resource|engineering/.test(text)&&/customer|implementation|support|custom/.test(text),owner=/come back to me|when i.?m away|decision.*delay|have to approve everything|without (my|me) (approval|say)/.test(text),money=/cost|expense|cash|profit|margin/.test(text);
...
function buildCandidates({latest,evidence,signals,openGaps,contradictions,questions,relationships,transcript}){
 const c=[],meaningful=meaningfulEvidence(evidence),seen=new Set((evidence||[]).map(e=>e.layer)),rel=relationshipReadiness({evidence,signals,relationships}),currentOwnerIsSubstantive=SUBSTANTIVE.test(latest);
 ...
 const material=materialPatternCandidates(latest,evidence,signals,questions);for(const x of material)c.push(x);
 ...
 if(/sales?.{0,25}(slow|down|fall|drop|declin|weak|soft|slip)|fewer customers|customers?\s*(are|have|are not)\s*(coming|buying)|revenue\s*(is|has)\s*(down|fall)/i.test(latest)&&!customerMixKnown)add(92,'clarify_active_signal',/customer/i.test(latest)?'When you say customers have changed, is it fewer customers coming, or existing customers making smaller purchases than before?':'When you say sales are down, is it mainly fewer customers, smaller purchases, or both?','I want to define the change clearly before testing what is causing it.',/customer/i.test(latest)?'customer':'revenue');
 ...
 // an actual candidate this turn -- not merely because a high-priority gap
 // existed on paper. A gap whose question already looks "asked" (near-
 // duplicate of a prior turn) but was never resolved must not be able to
 // silently starve the controller of every candidate: that would leave the
 // app with nothing to ask and nothing to call complete, going silent.
 if(material.length===0&&gapCandidatesAdded===0&&!(!rel.ready&&meaningful.length>=2))for(const layer of(adjacency[anchor]||LAYERS))if(!seen.has(layer))add(40,'material_gap',questionForLayer(layer,latest),'Check an adjacent business area only because no higher-value unresolved mechanism is currently available.',layer);
 return c;
}
