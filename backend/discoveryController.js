...
function materialPatternCandidates(latest,evidence,signals,questions){
 ...
 const econ=/revenue/.test(text)&&/(profit|margin|cash)/.test(text),service=/customization|customer-specific|custom work/.test(text)&&/support|implementation|engineering|delivery/.test(text),capacity=/capacity|team time|effort|resource|engineering/.test(text)&&/customer|implementation|support|custom/.test(text),owner=/owner|founder|myself|come back to me|when i.?m away|decision.*delay/.test(text),money=/cost|expense|cash|profit|margin/.test(text);
