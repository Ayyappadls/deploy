...
const RELATIONSHIP_TYPES=new Set(['CAUSES','CORRELATES_WITH','CONTRIBUTES_TO']);
...
function diagnosisReadiness(p){
 ...
 const contradictions=contradictionCount(p),supportedRelationships=relationships.filter(r=>supportedEvidence(r,p));
...
function blocked(stage,r){
 ...
 if(stage==='behavior')return{current:'No change mechanism is being prescribed yet.',friction:'Not established.',inertia:'Not established.',relapseRisk:'Premature intervention is the current risk.',wedge:'No behavioral wedge is defined until the required transition is verified.',required:'Verify the transition first.',capability:'Not assigned.',adoption:'Not applicable yet.',accountability:'Not assigned.',measure:'Not assigned.',methodologyGate:{status:'BLOCKED',reasons:r.reasons}};
 return{stakeholders:{owner:{means:'The owner needs a reliable diagnosis before acting.',understand:'The evidence is still being established; no unsupported decision is being communicated.',resist:'Not assessed yet.',action:'Continue evidence gathering.'}},decision:{decision:'Continue investigation before making a structural change.',reason:'The DLSMirror evidence gate is not satisfied.',owner:'Owner — continued evidence gathering',actions:[],expectedOutcome:'A defensible decision after the governing constraint is verified.',successMetric:'Diagnosis readiness becomes supported.',reviewDate:'after sufficient evidence is gathered'},methodologyGate:{status:'BLOCKED',reasons:r.reasons}};
}
function gateStage(stage,data,p){
 if(stage==='discover'||stage==='learning')return{data,gate:{status:'OPEN'}};
 const r=diagnosisReadiness(p);
 if(stage==='understand'){
  const relationships=arr(data?.relationships).filter(x=>supportedEvidence(x,p));
  const out={...data,relationships};
  if(!relationships.length)out.pattern={statement:'Material signals are present, but their relationships have not yet been sufficiently verified.',supportingRelationshipKeys:[],supportingSignalIds:[],confidence:'INSUFFICIENT_EVIDENCE'};
  out.methodologyGate={status:relationships.length?'OPEN_WITH_CAUTION':'BLOCKED',supportedRelationships:relationships.length,note:'Only relationships with explicit evidence support are presented.'};
  return{data:out,gate:out.methodologyGate};
 }
 ...
 if(!r.ready||!diagnosis||diagnosisGate!=='OPEN')return{data:blocked(stage,{...r,reasons:r.reasons.concat(!diagnosis?'diagnosis is missing':diagnosisGate!=='OPEN'?'diagnosis is not methodology-approved':[]) }),gate:{status:'BLOCKED',reasons:r.reasons.concat(!diagnosis?'diagnosis is missing':diagnosisGate!=='OPEN'?'diagnosis is not methodology-approved':[])}};
 if(stage==='transition'&&!p?.thesis)return{data:blocked(stage,{...r,reasons:r.reasons.concat('commercial thesis is missing')}),gate:{status:'BLOCKED',reasons:r.reasons.concat('commercial thesis is missing')}};
 if((stage==='behavior'||stage==='stakeholder')&&!p?.transition)return{data:blocked(stage,{...r,reasons:r.reasons.concat('approved transition is missing')}),gate:{status:'BLOCKED',reasons:r.reasons.concat('approved transition is missing')}};
